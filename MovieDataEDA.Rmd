...
author: "Jasmine Riggs"
date: "3/15/2020"
output: pdf_document
---
## Introduction
In this project, I really dive into the `movies` dataset from the `ggplot2movies` package. The data is from the internet movie database, [linked phrase](http://imdb.com). It's one of the largest movie databases today and is a collection of movie data supplied by studios and fans. There are 58,788 rows and 24 variables in this dataset.
```{r, echo=FALSE}
...
```
About 91.1% of movies have their budget included in this database and the rest, 8.9%, do not. The top 5 most expensive movies in this data set are "Spider-Man 2", "Titanic", "Troy", "Terminator 3: Rise of the Machines", and "Waterworld".
...
#vector of short films w/ length of 1 minute
moviesShort[moviesShort$length==1,"title"]
...
How many movies of each genre are there in this dataset?
```{r}
...
For this part, I wanted to combine multiple plots on one graph. For each genre (not including short films) plot movies from 1990-2005 and plot a function of the number of movies in this dataset of corresponding genre produced by year, for years from 1990 until the last year recorded.
